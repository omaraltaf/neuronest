...
**Implication for the codebase:** every agent prompt should explicitly reference NDBI principles, parent-coaching (not just parent-informing), and naturalistic/play-based framing over clinical drill framing. If you are asked to improve agent prompts, ground them in this section.

**THE CAST (consolidated 2026-07-17 — Omar's decision after the Santos/Eriksson overlap surfaced): exactly THREE personas.** **Dr. Lena Eriksson** is the family's ONE dedicated guide — intake interview, profile formulation, plan building, weekly focus, weekly check-ins, and in-the-moment session coaching are all her (the former Chen/Okafor/Santos roles were absorbed; their specialist expertise lives on inside the task prompts under her single harmonized identity: clinical psychologist + BCBA-D, 18 years, Oslo — see the header of `lib/agents/prompts.ts`). **Emma Blackwell** makes materials, never clinical. **Sunny** is the child's Child Zone companion. UI role tag for Eriksson is "your guide" everywhere. NEVER introduce a new clinical persona without an explicit product decision. Rationale: one relationship from day one (the vision's "dedicated in-home support worker" is singular); persona seams create "the system didn't listen" moments — the proof was a check-in whose content the weekly focus ignored.

**Check-in → immediate re-plan (2026-07-17):** completing a weekly check-in fires `/api/weekly-focus` POST with `trigger:'checkin'` (fire-and-forget from the check-in chat, which tells the parent the plan is being reworked). weekly-focus v7 receives the trigger, adds a TRIGGER context block, and the prompt instructs: reflect the fresh check-in visibly ("you just told me…"), refine rather than pivot mid-week. Check-ins are fed to planners ordered by **completed_at** (not created_at — a resumed old session completes with a stale created_at; learned 2026-07-17 when Omar finished a check-in started six weeks earlier).
...
**Norwegian materials — DONE (2026-07-17, the high-value slice of Phase 5):** materials, not UI chrome. The AAC router detects Norwegian requests (written in Norwegian or asking "på norsk") via a `language` field and the generators write everything in bokmål — verified live: "En valgtavle for matpakken med brødskive, eple og melk" → a 2×3 board titled "Matpakken min – Arya" with all named items + core words (jeg vil/mer/ferdig). Concepts are Norwegian and resolve-symbols v3 maps our 'no' to ARASAAC's locale **'nb'** (plain 'no' is a 400 — this silently sent every Norwegian concept to Imagen before) with æ/ø/å slug transliteration; verified ARASAAC hits for eple/hjelp/blåbær/brødskive/vaske hender. Per-child default language is editable on `/child` (Details → Language) and drives all agents (their prompts already say match the family's language). Materials carry their own language (revisions stay in it). NOT built (defer until asked): UI chrome translation (menus, buttons — Omar's family reads English fine).
