...
**Database (schema `neuronest`):** children, intake_sessions, child_profiles, plans, goals, app_state, documents, weekly_checkins, generated_content, session_logs, agent_state, notifications, story_images, milestones.
...
- The working solution: a **Supabase Edge Function** (`generate-story-images`, deployed via Supabase MCP/dashboard, NOT part of the git repo currently — it should be added to the repo under `supabase/functions/generate-story-images/index.ts` for version control, this hasn't been done yet) calls Gemini Imagen 4 from Supabase's servers, which have unrestricted network access.
...
1. **Add `supabase/functions/generate-story-images/index.ts` to the git repo** — currently only deployed via Supabase dashboard/MCP, not version controlled. Pull the current deployed source and commit it.
2. **Build the Weekly Planning Agent (§5.1)** — this is the single highest-leverage feature for turning NeuroNest from reactive to proactive. Use Fable 5 for the prompt design and initial implementation while available.
3. **Build the Goal Progression Engine (§5.2)** — second highest leverage. Requires real clinical reasoning about "what's the next reasonable goal" — this is exactly the kind of judgement call worth spending Fable 5's reasoning budget on now, then can run on a cheaper model once the reasoning template/prompt is well-designed.
4. **Wire Child Zone to active goals (§5.5)** — moderate effort, high payoff for making the platform feel unified rather than like disconnected features.
