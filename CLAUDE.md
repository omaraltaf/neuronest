...
**UX simplification: DONE (2026-07-05).** The evaluation and plan live in `UX_PLAN.md`; all of P1–P6 are implemented: bottom TabBar (`components/TabBar.tsx`) is the single navigation system (Today/Goals/Progress/Materials/Ask — Child Zone launches from Today); the dashboard is "Today" (hero focus card with the built-in practice loop: Today's 5 minutes → log → coaching, momentum line, ONE contextual banner); `components/PracticeLogger.tsx` is the universal quick-log used by Today and Goals (floating button); Progress is the records area (check-in summary + Reports & files); parent-words + persona role tags + ≥44px targets throughout.

**Card generation follows AAC standards (2026-07-05):** colours are the modified Fitzgerald Key by word class (people yellow #F59E0B, actions green #16A34A, describing blue #5B7FE8, things orange #F97316, social pink #DB2777, questions purple #7C3AED) — enforced in `CHILD_ZONE_CARDS_PROMPT` and the flashcard template — and cards get real Widgit/Boardmaker-style symbol images via the `generate-card-images` Edge Function (same Imagen pipeline + `story_images` cache as stories, instant ACK + waitUntil background work, emoji fallback in the UI). Spot-check new symbol batches — see the Imagen failure modes in §6.
