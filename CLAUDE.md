...
**⚠️ PENDING SUPABASE WORK (blocked 2026-07-13→16 by a dead session MCP link; account connector is fine — do these first thing once Supabase tools are available):** (1) deploy `supabase/functions/weekly-focus/index.ts` as v5 — the repo copy carries the hard rule that focus goals must come from working-on-now goals, the deployed v4 does not; (2) check whether the current week's `weekly_focus.primary_goal_ids` are all in_progress/emerging goals — if not, POST the Edge Function with `{child_id, force: true}` to regenerate; (3) AAC Studio Phase B per `AAC_STUDIO_PLAN.md` (aac_symbols migration + resolve-symbols Edge Function).

**AAC Studio (next major capability, designed 2026-07-16):** replace Omar's InPrint 3/Widgit workflow — prompt-driven creation of 9 AAC material types (communication boards, progressive sentence builders, visual timetables, comprehension, numbers, reward charts, word walls, matching games + existing flashcards). Full design in `AAC_STUDIO_PLAN.md` — READ IT before touching AAC code. Key decision: hybrid symbol sourcing — ARASAAC's open pictogram library first (~13k real AAC symbols, CC BY-NC-SA, API verified working; fetch from Edge Functions only, attribution required on prints), Imagen+QA generation as fallback for personalized concepts; symbols cached per-CONCEPT in `aac_symbols` (migration pending Supabase reconnect — Phase B blocked on it, Phase A is Vercel-only and buildable now).
