...
**AAC Studio (Phases A + B core LIVE, 2026-07-16):** replaces Omar's InPrint 3/Widgit workflow — prompt-driven AAC material creation. Full design + phase status in `AAC_STUDIO_PLAN.md` — READ IT before touching AAC code. What's live: `aac_symbols` table (per-CONCEPT symbol cache, unique concept+language, authenticated read / service-role-only writes) + `resolve-symbols` Edge Function v2 (ARASAAC-first — plain keyword search returned a CAR WASH pictogram for "wash hands", so the top 3 ranked candidates are each vision-QA'd for semantic match before caching; Imagen+QA fallback for personalized concepts; same conventions as the other Edge Functions: manual redeploy, x-cron-secret auth). App surface: "Describe what you need" free-text box at the top of Materials → `/api/aac-studio` (Emma routes free text to a material type, generates with structured output from `lib/agents/aacTemplates.ts`, fires resolve-symbols server-side — the browser never sees the cron secret) → three new content types `comm_board` / `sentence_builder` / `visual_timetable` with viewers + print layouts in `app/content/aacViewers.tsx` (kept OUT of page.tsx deliberately, per the file-integrity warning above; ARASAAC attribution footer renders whenever ARASAAC symbols appear). Verified end-to-end against Arya's data. Remaining: Phase B tail (switch flashcards + Child Zone cards to concept-keyed symbols, backfill story_images card symbols), Phase C types, Phase D polish.
