...
- **Supabase Auth "Site URL" pointed at the STALE domain too** (found 2026-07-17 when the first guardian invite email delivered a perfectly valid session to the dead app): every auth email link (invite, recovery, signup confirmation) redirects to Site URL unless the requested redirect_to is allowlisted. Fix lives in Supabase Dashboard → Authentication → URL Configuration (Site URL `https://neuronest-nine.vercel.app`, Redirect URLs `https://neuronest-nine.vercel.app/**`) — Omar was asked to change it 2026-07-17; if auth links misbehave, verify this FIRST. Interim rescue used: set the invitee's password directly (`update auth.users set encrypted_password = extensions.crypt(..., gen_salt('bf', 10))`) so they can sign in without any link.
