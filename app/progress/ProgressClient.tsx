    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-gray-400">{childName}&apos;s journey</div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4 pb-12">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <div key={s.label} className="bg-white rounded-2xl border border-gray-100 p-3 text-center">
              ...
              <div className="text-[9px] text-gray-400 font-semibold leading-tight">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          <div className="text-xs font-black text-gray-900 mb-3">Activity — last 30 days</div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          <div className="text-xs font-black text-gray-900 mb-3">All goals</div>
          <div className="space-y-2">
            {goals.map(goal => (
              <div key={goal.id as string} className="flex items-center gap-3">
                ...
                <div className="flex-1 text-xs text-gray-700 truncate">{goal.label as string}</div>
                <div className="text-[10px] text-gray-400 capitalize flex-shrink-0">
        ...
        {logs.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
            <div className="text-xs font-black text-gray-900 mb-3">Recent sessions</div>
            <div className="space-y-2">
              {logs.slice(0, 15).map(log => (
                <div key={log.id as string} className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-0">
                  <div className="w-7 h-7 rounded-xl bg-violet-50 flex items-center justify-center text-sm flex-shrink-0">
                    {log.rating ? ['','😰','😕','😐','😊','🌟'][log.rating as number] : '✓'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-semibold text-gray-800 truncate">{log.activity_title as string}</div>
                    {!!log.notes && <div className="text-[10px] text-gray-400 mt-0.5 line-clamp-1">{log.notes as string}</div>}
                  </div>
